-- =====================================================
-- ClickHouse: 企业行业位阶（AI 数据，对照全球/中国基准，3维）
-- 基于 tmp_enterprise_patent_ai（由 step2 AI 流程创建）
-- 逻辑与 enterprise_industry_rank_v3.sql 完全一致，临时表和结果表均以 _ai 后缀区分
-- 1. 取企业专利数前 3 IPC 小类，按数量赋权
-- 2. 对照 industry_quantile 分位数插值得到百分位
-- 3. 3 维（密度/引用/家族）取均值→综合位阶
-- 输出表：XiaoSu.enterprise_industry_rank_ai（独立于 CCB）
-- =====================================================

-- ============ Step 1: 企业 × 小类 指标（仅该小类下专利）============
DROP TABLE IF EXISTS XiaoSu.tmp_ent_subclass_metrics_ai;

CREATE TABLE XiaoSu.tmp_ent_subclass_metrics_ai
ENGINE = MergeTree()
...
CREATE TABLE XiaoSu.tmp_ent_top3_subclass_ai
ENGINE = MergeTree()
ORDER BY (enterprise_name, subclass)
AS SELECT
    enterprise_name,
...
FROM (
    ...
    FROM XiaoSu.tmp_ent_subclass_metrics_ai
)
WHERE rn <= 3;
...
CREATE TABLE XiaoSu.enterprise_industry_rank_ai
(
...
CREATE TABLE XiaoSu.tmp_ent_subclass_pct_ai
(
    enterprise_name String,
    scope String,
    subclass String,
...
INSERT INTO XiaoSu.tmp_ent_subclass_pct_ai
SELECT
    t3.enterprise_name,
    q.scope,
    t3.subclass,
...
FROM XiaoSu.tmp_ent_top3_subclass_ai AS t3
INNER JOIN XiaoSu.industry_quantile q ON t3.subclass = q.ipc_subclass
INNER JOIN XiaoSu.tmp_ent_subclass_metrics_ai sub ON t3.enterprise_name = sub.enterprise_name AND t3.subclass = sub.subclass
INNER JOIN XiaoSu.enterprise_eval_result_ai eval ON t3.enterprise_name = eval.enterprise_name;

-- ============ Step 4b: 按企业、scope 聚合（加权平均）============
INSERT INTO XiaoSu.enterprise_industry_rank_ai
SELECT
    enterprise_name,
    scope,
    main_subclass,
...
FROM (
    ...
    FROM XiaoSu.tmp_ent_subclass_pct_ai
